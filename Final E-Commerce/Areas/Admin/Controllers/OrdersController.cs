...
namespace Final_E_Commerce.Areas.Admin.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        public async Task<IActionResult> Confirm(int? id)
        {
            ...
            {
                Products? dbProduct = await _context?.Products?.FirstOrDefaultAsync(p => p.Id == product.ProductId);
                if (dbProduct.DiscountPercent > 0)
                {
                    dbProduct.Profit += dbProduct.DiscountPrice * product.Count;
                }
                else
                {
                    dbProduct.Profit += dbProduct.Price * product.Count;
                }
                dbProduct.Sold += product.Count;
                //endirimde olmadiqda:
